"""Outbound Claude API metering for LEC -- wraps Anthropic messages.create()
and logs token counts, cost, and request context to LEC's OWN claude_api_usage
table.

This is LEC's calibration-spend meter. It mirrors the Rising Compass meter but
with the CORRECT current Opus pricing ($5 / $25 per MTok) -- RC's meter used the
stale $15 / $75 rate (overcounting ~3x). After the migration, calibration spend
lives HERE (LEC's DB); RC's meter only covers RC's non-calibration Anthropic
calls.
...
(ok=0, error=...) so prompt-explosion bugs and Anthropic outages show up as
cost-zero rows. The usage row is written inline via a short-lived ORM session;
write errors are swallowed -- usage logs are telemetry, not a correctness signal.
"""
...
# CORRECT current Opus rate is $5 in / $25 out (cache write 5m = 1.25x input =
# $6.25; cache read = 0.1x input = $0.50). This is the fix over RC's stale
# $15 / $75 Opus rate. pricing_source on each row records which entry was used;
# "fallback_opus" means the model wasn't recognized and we charged Opus rates.
...
def _resolve_rates(model: str) -> tuple[Rates, str]:
    ...
    Matches by family substring so new minor versions inherit the family rate
    without a code change. Unknown models fall back to Opus pricing (the
    conservative upper bound) with a flagged source.
    """
    m = (model or "").lower()
    if "opus" in m:
...
def _build_row(
    *,
    ...
    if context:
        try:
            # Cap individual values so a runaway text blob doesn't bloat the log.
