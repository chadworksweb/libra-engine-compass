"""Motion Desk ops for the lecg- venue -- filing, validation, lifecycle.

Faithful port of RC's services/motions.py, re-keyed from User to Citizen and
with ONE substantive change: target validation reads the canonical constitution
(governance/lecg_constitution.load_constitution) instead of RC's core.json. That
is the whole point of Part 2 -- the venue validates motions against the law it
governs, not against a copy borrowed inside a product.

Motions deliberate the framework (tenets, rules, modifiers, methodology), never a
song. Tier-2 (verified human, a shared identity credential) gates filing; that
gate lives in the router dependency (require_verified_citizen), so these service
functions receive an already-verified citizen. Status lifecycle is one-way:
...
from governance.lecg_models import Citizen, Motion

logger = logging.getLogger(__name__)


# motion_type taxonomy (identical to RC)
...
def _validate_citations(raw: Optional[list]) -> Optional[str]:
    ...
    for item in raw:
        ...
        if len(cleaned) > MAX_CITATIONS:
            raise HTTPException(status_code=422, detail=f"Maximum {MAX_CITATIONS} citations per motion")
    if not cleaned:
        return None
...
def _load_constitution():
    """Lazy import + load so test/migration paths stay independent of the
    constitution import chain at module load."""
...
def _known_rule_ids() -> set[str]:
    c = _load_constitution()
    return {r.get("id") for r in c.get("rules", []) if r.get("id")}


def _known_modifier_ids() -> set[str]:
    c = _load_constitution()
    return {m.get("id") for m in c.get("modifiers", []) if m.get("id")}


def _validate_target(motion_type: str, target_kind: Optional[str], target_ref: Optional[str]) -> tuple[Optional[str], Optional[str]]:
    """Return the cleaned (kind, ref) tuple. Raises 422 on mismatch against the
    canonical constitution."""
    if motion_type in OPEN_TYPES:
        if target_kind or target_ref:
            raise HTTPException(status_code=422, detail=f"{motion_type} motions do not take a target")
        return None, None

    if motion_type in NEW_TYPES:
        if target_ref:
            raise HTTPException(status_code=422, detail=f"{motion_type} motions propose a new artifact -- no target_ref")
        implied_kind = "tenet" if motion_type == "new_tenet" else "rule"
        if target_kind and target_kind != implied_kind:
            raise HTTPException(status_code=422, detail=f"{motion_type} implies target_kind='{implied_kind}'")
        return implied_kind, None

    if motion_type in {"amend_tenet", "remove_tenet"}:
        kind = "tenet"
        if target_kind and target_kind != "tenet":
            raise HTTPException(status_code=422, detail=f"{motion_type} requires target_kind='tenet'")
    elif motion_type in {"amend_rule", "remove_rule"}:
        kind = (target_kind or "rule").strip().lower()
        if kind not in {"rule", "modifier"}:
            raise HTTPException(status_code=422, detail=f"{motion_type} requires target_kind in (rule, modifier)")
    else:
        raise HTTPException(status_code=422, detail=f"Unknown motion_type: {motion_type}")

    if not target_ref:
        raise HTTPException(status_code=422, detail=f"{motion_type} requires target_ref (the id within target_kind)")
    ...
    if kind == "tenet":
        if ref.lower() not in _known_tenet_ids():
            raise HTTPException(status_code=422, detail=f"target_ref '{ref}' does not match any tenet in the constitution")
        ref = ref.lower()
    elif kind == "rule":
        if ref not in _known_rule_ids():
            raise HTTPException(status_code=422, detail=f"target_ref '{ref}' does not match any rule in the constitution")
    elif kind == "modifier":
        if ref.lower() not in _known_modifier_ids():
            raise HTTPException(status_code=422, detail=f"target_ref '{ref}' does not match any modifier in the constitution")
        ref = ref.lower()
    return kind, ref
...
def file_motion(
    ...
    """File a motion. The citizen is already Tier-2 verified (router dependency)."""
    if motion_type not in VALID_MOTION_TYPES:
        raise HTTPException(status_code=422, detail=f"motion_type must be one of {sorted(VALID_MOTION_TYPES)}")
    ...
    reasoning_clean = _normalize_text(reasoning, "reasoning", min_len=MIN_REASONING_LENGTH, max_len=MAX_REASONING_LENGTH)
    ...
    motion = Motion(
        ...
        filed_by_citizen_id=citizen.id,
        status="filed",
    )
    db.add(motion)
    db.commit()
    db.refresh(motion)
    logger.info("Motion filed: %s id=%s by citizen=%s kind=%s ref=%s", motion_type, motion.id, citizen.id, kind, ref)
    return motion
...
def transition_status(
    ...
    resolution_summary required (10+ chars) for any terminal transition. NOTE:
    ratification's WRITE-BACK to the constitution is NOT done here -- the admin
    router calls lecg_amendments.ratify_motion, which transitions AND amends the
    law atomically."""
    ...
    if motion.status == "filed":
        if new_status not in {"in_deliberation", "rejected"}:
            raise HTTPException(status_code=409, detail="From filed, only in_deliberation or rejected are allowed")
    elif motion.status == "in_deliberation":
        if new_status not in {"ratified", "rejected", "covered"}:
            raise HTTPException(status_code=409, detail="From in_deliberation, only ratified/rejected/covered are allowed")
    else:
        raise HTTPException(status_code=409, detail=f"Motion already resolved (status={motion.status})")

    is_resolution = new_status in {"ratified", "rejected", "covered"}
    if is_resolution:
        if not resolution_summary or len(resolution_summary.strip()) < 10:
            raise HTTPException(status_code=422, detail="resolution_summary required (10+ chars) when closing a motion")
        motion.resolution_summary = resolution_summary.strip()
        motion.resolved_at = datetime.utcnow()
        motion.resolved_by_admin = admin_username

    motion.status = new_status
    db.commit()
    db.refresh(motion)
    logger.info("Motion %s transitioned to %s by admin=%s", motion.id, new_status, admin_username)
    return motion


# ---------- serialization ----------

def motion_to_dict(db: Session, motion: Motion, *, include_filer_handle: bool = True) -> dict:
    citations: list = []
    if motion.citations:
        try:
    ...
    if include_filer_handle and motion.filed_by_citizen_id:
        filer = db.query(Citizen).get(motion.filed_by_citizen_id)
        if filer is not None:
            out["filed_by_handle"] = filer.handle
            out["filed_by_anon_id"] = filer.anon_id
            # Filing required Tier-2 at file time, so an existing motion's filer
            # was a verified human by construction (the gate cannot be bypassed).
            out["filed_by_verified"] = True
