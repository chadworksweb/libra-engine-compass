"""Motion Desk public API (lecg-). Faithful port of RC routers/motions.py,
re-keyed to Citizen + the shared-Clerk Tier-2 gate.

Surface:
  POST /api/motions          file a motion (Tier-2 verified citizen)
...
from governance.lecg_auth import require_verified_citizen
...
class FileMotionIn(BaseModel):
    motion_type: str = Field(..., description="amend_tenet | new_tenet | remove_tenet | amend_rule | new_rule | remove_rule | process")
    target_kind: Optional[str] = Field(None, description="tenet | rule | modifier. Omit for process or new_* motions.")
    target_ref: Optional[str] = Field(None, description="Id within target_kind, e.g. 'violet-01' / 'R1' / 'contamination'. Required for amend_* / remove_*.")
    claim: str = Field(..., min_length=1, max_length=motions_svc.MAX_CLAIM_LENGTH)
    reasoning: str = Field(..., min_length=motions_svc.MIN_REASONING_LENGTH, max_length=motions_svc.MAX_REASONING_LENGTH)
...
def file_motion(
    data: FileMotionIn,
    citizen: Citizen = Depends(require_verified_citizen),
