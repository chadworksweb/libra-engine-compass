"""Deliberation Chamber public API (lecg-). Faithful port of RC routers/chamber.py.

Surface:
  POST /api/motions/{id}/arguments              post an argument (Tier-2 verified)
...
from governance import lecg_chamber as chamber_svc
...
class PostArgumentIn(BaseModel):
    post_type: str = Field(..., description="argument_for | argument_against | rebuttal | citation | clarification")
    parent_id: Optional[int] = Field(None, description="Required for rebuttal posts, NULL otherwise.")
    summary: str = Field(..., min_length=1, max_length=chamber_svc.MAX_SUMMARY_LENGTH)
    body: str = Field(..., min_length=chamber_svc.MIN_BODY_LENGTH, max_length=chamber_svc.MAX_BODY_LENGTH)
...
@router.get("")
def list_arguments(motion_id: int, db: Session = Depends(get_db)):
    motion = _get_motion_or_404(db, motion_id)
    rows = (
...
@router.get("/{arg_id}")
def get_argument(motion_id: int, arg_id: int, db: Session = Depends(get_db)):
    motion = _get_motion_or_404(db, motion_id)
