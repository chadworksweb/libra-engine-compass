/* === The Tenets -- render the universal tenets (tiers + tenets + procedural) + organ from window.TENETS_DATA === */
...
(() => {
  ...
  // --- Data source -------------------------------------------------
  // The universal tenets (de-musked), baked into tenets-data.js as
  // window.TENETS_DATA. No API call: this is the Libra Engine tenets
  // view itself, not a live product feed.
  async function fetchTenets() {
    if (!window.TENETS_DATA) throw new Error('tenets data not loaded');
    return window.TENETS_DATA;
  }
